...
class BC{
	...
	public boolean bcentails(){
		...
		while(!agenda.isEmpty()){
			...
			//check whether it belongs to environment variables array which you have to create
			entailed.add(q);

			// if this element is a fact then we don't need to go further
			if (!facts.contains(q)){
				// .. but it isn't so..
				...
				for(int i=0;i<clauses.size();i++){
					...
					if (conclusionContains(clauses.get(i),q)){
						...
						for(int j=0;j<temp.size();j++){
							// add the symbols to a temporary array
							//check whether it belongs to environment variables array which you have to create
							p.add(temp.get(j));
						}
					}						
				}
				// no symbols were generated and since it isn't a fact either 
				// then this symbol and eventually ASK  cannot be implied by TELL
				if (p.size()==0){
					return false;
				}
				else{
